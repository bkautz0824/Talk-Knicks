...
export async function getLivingListById(id) {
  const response = await fetchAPI(`living-lists/${id}?populate[items][populate][0]=attributes&populate[items][populate][1]=accolades&populate[items][populate][2]=stats`);
  return response;
}
